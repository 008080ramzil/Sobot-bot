...
async def handle_urge_button(message: types.Message):
    ...
    try:
        ...
        ai_support = ai_motivator.generate_urge_support_message(current_day, recent_urges)
...
async def ai_motivation(message: types.Message):
    ...
    try:
        ai_message = ai_motivator.generate_personalized_motivation(current_day, user_context)
        await message.answer(f"✨ {ai_message}", reply_markup=get_main_keyboard())
    except Exception as e:
        logging.error(f"AI motivation error: {e}")
        if current_day in MOTIVATIONS_BY_DAY:
            await message.answer(MOTIVATIONS_BY_DAY[current_day], reply_markup=get_main_keyboard())
        else:
            await message.answer("💪 Ты молодец! Продолжай в том же духе!", reply_markup=get_main_keyboard())
...
async def ai_question(message: types.Message):
    ...
    try:
        ai_question = ai_motivator.generate_reflection_prompt(current_day)
        ...
        await message.answer(f"🤔 {question}", reply_markup=get_main_keyboard())
...
async def handle_responses(message: types.Message):
    ...
    if text in ["да", "нет", "не очень", "не особо"]:
        ...
        if text in ["нет", "не помогло"]:
            ...
            new_recommendation = random.choice(available_recs) if available_recs else random.choice(EXERCISE_RECOMMENDATIONS)
        ...
        try:
            ...
            if 1 <= current_day <= 28:
                ...
                try:
                    ...
                    ai_motivation = ai_motivator.generate_personalized_motivation(current_day, user_context)
